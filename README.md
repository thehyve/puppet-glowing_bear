...
cp -r stdlib archive apache /etc/puppet/modules/
```

### Install the `glowing_bear` module
Copy the `glowing_bear` module repository to the `/etc/puppet/modules` directory:
```bash
...
The tests are automatically run on our Bamboo server: [PUPPET-PUPPETGB](https://ci.ctmmtrait.nl/browse/PUPPET-PUPPETGB).
